...
const POSTER_TUTORIAL_URL = 'https://www.youtube.com/embed/iQtFMsI_jLI?autoplay=1&mute=1&origin=http://localhost:3000';
...
const buildProductPrompt = (
  ...
  const base =
    `${data.resolution} ${data.photoStyle} marketing poster illustration of a ${data.productType} as the main hero object. ` +
    `Brand Style: ${data.brandStyle}. Material: ${data.mainMaterial}. Color Palette: ${data.color}. ` +
    `Key Product Details: ${data.uniqueDetails}. Layout & Composition: ${data.angle}, ${data.focus}. The product is shown ${data.actionState}. ` +
    `Environment & Worldbuilding: ${data.background}, with ${data.secondaryElements}. Atmosphere: The mood is ${data.mood}, supported by ${data.lighting} and ${data.shadows}. `;

  const referenceInstruction = hasReferenceImage
    ? 'IMPORTANT: A reference PRODUCT PHOTO is provided. In the poster illustration, the hero product MUST keep the exact same physical design as the reference photo: same overall shape and silhouette, same proportions, same key components and attachments, and same main color scheme. Even if the rest of the scene becomes flat, cartoon, or highly stylized, the base product silhouette and layout must stay identical to the reference.'
    : 'IMPORTANT: If a reference PRODUCT PHOTO is provided, the hero product in the poster must strictly match that reference in shape, proportions, and main color layout, even when drawn in an illustrated or graphic style, so the design stays consistent across all poster variations.';

  const consistencyInstruction =
    'IMPORTANT: Across ALL poster variations, the product design must NEVER change. Do NOT change the core model, silhouette, proportions, logo placement, or main color scheme between posters. Only layout, typography, surrounding characters, scenery, and graphic elements may change.';

  const qualityInstruction =
    `The final image must be a finished marketing poster layout with clean, professional graphic design. Text and typography are allowed as part of the design, but any visible text must be strictly product-related (brand name, menu item names, short marketing slogans, prices, or flavor descriptors only). There must be no random watermarks, AI signatures, internal style labels, or unrelated UI elements. The composition must strictly follow the ${aspectRatio} aspect ratio.`;
...
// Poster photoshoot descriptions (main 16 shots)
const PHOTOSHOOT_DESCRIPTIONS: string[] = [
  // I. Brutal Appetite · Hero Food Posters
  'Warm, inviting hero food poster: several dishes arranged in perspective along a curved table or stage; hero product closest to viewer, supporting items behind. Background: smooth orange/warm gradient with soft bokeh and subtle restaurant hints. Big stacked title at top, small bubbles around dishes for menu names, flavor variants, and one clear round price badge. Do NOT write the word "Label" or any technical style name in the poster.',
  'Hero food poster as a modular wall: build rectangular panels using 2–3 brand colors (e.g., mustard yellow, deep red, cream). Some panels show tight product crops, others full shots, others text only. Typography is big, dense, sometimes rotated to fill space. All text should be menu names, short taglines, and prices related to the product—no label words or style codes.',
  'Menu-board poster split into two or three main dishes. Divide into strong blocks/shapes or ribbons; each block has one plate shot from 3/4 top view with glossy appetizing lighting. Around each plate add a short description and round/pill price tag. Big title at the top can be promos like "New Menu" or "Special Deals", but never write "Label" or internal style names.',
  'Minimal premium food poster with a single hero plate centered. Background is two clean colors split vertically/diagonally; plate sits center with soft shadow. Thin text may curve around the plate; elegant title and big price near the bottom. Palette is limited and sophisticated so food texture stays focus; no technical text like label names or style codes.',

  // II. Playful Doodle & Pop Color
  'Playful Doodle & Pop Color · Cartoon Food Parade. Style Focus: Over-the-top 2D illustration. Thick black outlines, flat bold colors, busy with funny side characters/icons orbiting the hero product. Giant chunky hand-drawn slogan in the middle. Loud, playful, chaotic (Japanese/Korean snack vibe). Hero product shape/proportions stay exactly like the reference, simplified into flat cartoon.',
  'Playful Doodle & Pop Color · Roadtrip Burst. Style Focus: Dynamic cartoon with motion. Wide 2D scene where the hero product rides or blasts forward, with smoke clouds, speed lines, and urban/festival elements. Large curved comic lettering follows motion path. Flat, saturated, outlined; no realistic camera. Hero silhouette identical to the real object; world exaggerated and playful.',
  'Playful Doodle & Pop Color · Comic Triptych. Style Focus: 2D comic strip. Split into 3–4 horizontal panels, each with hero product in a mini-scene with cute characters, speech bubbles, action doodles. Big comic typography (travel/adventure feel) overlaps frames. Bright limited colors, thick clean line art, hero shape consistent in every panel—only pose/context changes, never design.',
  'Playful Doodle & Pop Color · Limited-Palette Market. Style Focus: Graphic/screen-print feel. Very limited bright palette (e.g., yellow background, blue line art, one accent). Hero product inside a cluster of related items (grocery cart/market display) drawn with thick lines and flat fills. Huge block heading at top announces brand/product. Hero stays recognizable with same silhouette and key details, rendered in flat illustration.',

  // III. Modern Dimension (Promo & Atmosphere)
  'Modern Dimension · Sky Portal. Style Focus: dreamy, optimistic hero poster. Main product/character slightly in front of a large rectangular/arched frame floating in a soft sky with clouds. Around the frame, add floating elements like butterflies, small stars, or brand icons. Main title in large flowing script across the sky; event/promo details neatly at the bottom with tidy icons.',
  'Modern Dimension · Radial Plate Spotlight. Style Focus: bold circular food composition. Place hero plate centered within a decorative circle border. Behind it, repeat a big product word diagonally at low opacity as a pattern. Use 1–2 strong accent colors as big diagonals in the background. On the right or bottom, arrange small text blocks for CTA, address, or contact—modern and structured.',
  'Modern Dimension · Neon Deal Night. Style Focus: dark neon sale poster. Background: dark brick or night gradient; place main products centered with slight floor reflection. Behind, build a big neon-signage heading (e.g., BLACK FRIDAY/BIG SALE) with glowing edges. Add discount badges, voucher tags, percent icons around; all glowing and high contrast with background.',
  'Modern Dimension · Spotlight Stage. Style Focus: 3D stage promo with spotlights. Create a tiered stage/podium center; main product highest, supporting products on lower levels. Overhead spotlights create light beams and strong highlights; a bit of confetti or bokeh in background. Big promo title at top; voucher/benefit/promo mechanics in clean strips at bottom.',

  // IV. Creative & Editorial (Artistic)
  'Creative Frame / Collage · Memory Grid. Style Focus: nostalgic monochrome collage. Arrange multiple product/scene photos in an asymmetric grid on a subtle textured background. Most photos in black-and-white or soft tones, with thick white frames. Leave one or two vertical panels for theme text and big year numbers; small script title about the story/collection in a corner. All text must relate to the story/product name, not technical labels.',
  'Creative Frame / Collage · Gallery Wall. Style Focus: framed portrait wall. Place a sofa or main furniture at the bottom; hero product/character sits or rests on it. Behind, a patterned wallpaper wall filled with frames of varied sizes containing product variations or supporting characters. Center composition gets a big series/campaign title with strong brand-relevant typography.',
  'Creative Frame / Collage · Polaroid Board. Style Focus: playful pinned-card layout. Poster filled with polaroid/ID-style colorful cards, tilted and overlapping on a neutral background. Each card shows one product/character; small space at bottom for name/role. Add paperclip, tape, pen doodles, arrow lines connecting cards. Center area left for a big headline describing theme/campaign title.',
  'Creative Frame / Collage · Shattered Panel Hero. Style Focus: dynamic cutout collage. One big hero figure/product in center, cut into several rectangular panels slightly offset like a broken collage. Behind panels, show small supporting scenes or environmental textures. Top or bottom: product/series title with strong type plus a short subtitle explaining key value.',
];

// Extended set for poster Extend Generate (additional 16 shots)
const POSTER_EXTENDED_DESCRIPTIONS: string[] = [
  'Elegant Noir · Editorial Cover Frame. Style Focus: fashion magazine cover. Use black-white textured background with supporting figures/products styled like editorial photos. In the center, place a colored box/frame containing the hero product photo in full color to pop. At the top, build a large masthead like a magazine title using the brand or series name; scatter small text blocks on left/right for feature highlights, benefits, or short taglines that stay relevant to the product.',
  'Elegant Noir · Black & Gold Mirror. Style Focus: luxe black-gold symmetry. Poster with deep black background and gold decorative lines framing the composition. Position two identical products/characters symmetrically (top-bottom or left-right) like a mirror around the center. In the middle, write the product/campaign name vertically in tall gold art-deco letters. Add round badges or small ornaments for promo info, event date, or logo—keep it premium.',
  'Elegant Noir · Giant Noir Letters. Style Focus: minimal typographic noir. Place one main product/character sitting or leaning in front of huge letters arranged vertically/diagonally in the background (forming brand/series/keyword). Palette is very limited (black, gray, a bit of white) with strong contrast between figure and giant letters. Supporting text only a few short lines to explain slogan/benefit/message.',
  'Elegant Noir · Dark Night Club. Style Focus: moody event poster. Deep black background with a soft glow around the hero product/character center, as if in a night club or futuristic dark room. Behind it, repeat a big brand/event word as layered outline text to add depth. Top: bold white event/series title; bottom: neatly arranged essentials (date, time, location, contact) relevant to the campaign.',
  'Organic Origin · Green Info Grid. Style Focus: fresh agricultural collage. Build a grid composition of 6–9 square panels mixing field photos, product close-ups, and green-white text blocks. Each panel may show a product, farmer, or fresh ingredient; text panels carry short titles, benefit points, and a simple call-to-action relevant to the campaign. Use leaf elements, agriculture icons, and a contact bar at the bottom to reinforce a natural, professional theme.',
  'Organic Origin · Farmer Field Tribute. Style Focus: hero in the field. One main figure stands in the middle of green crops with broad sky behind. Above/behind, repeat natural-toned words multiple times as layered typography; some leaves float in front for liveliness. Bottom: text block with appreciation or a short story about how the product supports nature/farmers, plus small area for logo and brand info.',
  'Organic Origin · Seasonal Specials Row. Style Focus: bold food lineup on a color block. Arrange bowls or small containers of various dishes/variants diagonally on a warm surface (yellow/orange). Background behind is a strong green field with stacked big title announcing new menu or seasonal specials. Big text only states campaign/promo; leave small space near the food row for prices or flavor notes.',
  'Organic Origin · Botanical Product Hero. Style Focus: premium natural skincare. Place the main product bottle on mossy wood/stone with a soft green, forest-blur background. Elegant typography for product name and main claim at upper center; a simple discount badge or CTA ("shop now"/"discover more") near the bottom. Add thin arrows/lines pointing to key packaging elements—informative yet clean and natural.',
  'Dynamic Motion · Stadium Energy Squad. Style Focus: epic sports energy poster. Build a track/arena scene with one giant main product in the center and several young athletes/characters standing confidently in front. Surround with buildings/stadium and speed lines aiming toward the product to convey motion. Big campaign title curved at top, with particle or product-ingredient elements flying fast in the foreground.',
  'Dynamic Motion · Fisheye Jump Burst. Style Focus: extreme perspective action. Show a figure leaping toward the camera with wide/fisheye angle so feet/shoes are very close. Around the body, add colorful doodles: speed lines, small icons, bursts from the shoes/product. Big campaign typography at top, slightly tilted to boost motion.',
  'Dynamic Motion · Berry Orbit Hero. Style Focus: floating flavor tunnel. Place the main product centered, slightly tilted as if spinning. Surround with fruits/toppings orbiting toward the camera in a circular path; use a strong solid background color for contrast. Flavor/variant name text is large at top or side, bold and readable.',
  'Dynamic Motion · Citrus Rush Pack. Style Focus: fast-moving flavor splash. Put a vertical product pack in the center, crisp and clean, with fresh fruit slices swirling front and back as if drawn toward the product. Background is a warm yellow-orange gradient with subtle fruit/bubble patterns for freshness and energy. Product name and short claim near top; flavor/size info below—stay campaign-focused, no extra technical text.',
  'Graphic Color Block · Fashion Spotlight Blocks. Style Focus: bold fashion color-block. Place the main figure/product on the right or center, standing before stacked geometric shapes (squares/triangles) in contrasting colors. On the left, fill most of the column with a big white headline stating the core campaign message or product offer. Small brand logo/icon can sit in a corner—focus on the shape/color combo.',
  'Graphic Color Block · Playful Trail Illustration. Style Focus: graphic story path. Illustrate a rising trail/hill as a big ribbon curving to the horizon, surrounded by simplified trees and cheerful supporting characters. A main character/product moves along the path as the focus. Use very simplified clouds/trees/flowers with solid colors; big headline follows the path curve at the bottom to reinforce motion.',
  'Graphic Color Block · Summer Grid Campaign. Style Focus: beach product grid. Arrange several square panels in a grid, each showing the product in a different beach vibe: on sand, near waves, beside sunglasses, under colorful umbrellas. Blue sky, beige sand, and product orange become large color blocks. Leave big space in some panels for summer promo text/discount/CTA relevant to the product.',
  'Graphic Color Block · Neon Portrait Overlays. Style Focus: vibrant gradient overlays. Use a photo of a person/character as the main subject overlaid with semi-transparent color blocks and strong gradients (e.g., cyan, magenta, yellow). Shapes can be rectangles or circles cutting across parts of the face/body for modern layering. Place bold typography in empty areas with event title, date, and short campaign details—no technical text.',
];

// Poster shot labels (main 16) - ringkas
const PHOTOSHOOT_LABELS: string[] = [
  '1 · Brutal Appetite · Parade',
  '1 · Brutal Appetite · Modular Wall',
  '1 · Brutal Appetite · Twin Hero',
  '1 · Brutal Appetite · Split Plate',
  '2 · Playful Doodle · Cartoon Parade',
  '2 · Playful Doodle · Roadtrip',
  '2 · Playful Doodle · Triptych',
  '2 · Playful Doodle · Limited Market',
  '3 · Modern Dimension · Text Wall',
  '3 · Modern Dimension · Isometric Podium',
  '3 · Modern Dimension · Layered Depth',
  '3 · Modern Dimension · Shadow Play',
  '4 · Creative Collage · Memory Grid',
  '4 · Creative Collage · Gallery Wall',
  '4 · Creative Collage · Polaroid',
  '4 · Creative Collage · Shattered Panel',
];

// Extended poster labels (16) - ringkas
const PHOTOSHOOT_EXTENDED_LABELS: string[] = [
  '5 · Elegant Noir · Golden Rim',
  '5 · Elegant Noir · Marble Plinth',
  '5 · Elegant Noir · Luxury Bokeh',
  '5 · Elegant Noir · Reflection Pool',
  '6 · Organic Origin · Ingredient',
  '6 · Organic Origin · Sun Rays',
  '6 · Organic Origin · Rustic Table',
  '6 · Organic Origin · Water Freshness',
  '7 · Dynamic Motion · Splash Crown',
  '7 · Dynamic Motion · Ingredient Boom',
  '7 · Dynamic Motion · Mid-Air',
  '7 · Dynamic Motion · Impact',
  '8 · Graphic Color · Spotlight',
  '8 · Graphic Color · Trail',
  '8 · Graphic Color · Summer Grid',
  '8 · Graphic Color · Neon Overlay',
];

type AngleGroupDef = { id: string; title: string; subtitle: string };

// Poster style groups (numeric like Generate Product)
const ANGLE_GROUPS_BASE: AngleGroupDef[] = [
  { id: '1', title: '1 · Brutal Appetite', subtitle: 'Shots 1–4' },
  { id: '2', title: '2 · Playful Doodle', subtitle: 'Shots 5–8' },
  { id: '3', title: '3 · Modern Dimension', subtitle: 'Shots 9–12' },
  { id: '4', title: '4 · Creative Collage', subtitle: 'Shots 13–16' },
  { id: '5', title: '5 · Elegant Noir', subtitle: 'Shots 17–20' },
  { id: '6', title: '6 · Organic Origin', subtitle: 'Shots 21–24' },
  { id: '7', title: '7 · Dynamic Motion', subtitle: 'Shots 25–28' },
  { id: '8', title: '8 · Graphic Color Block', subtitle: 'Shots 29–32' },
];

const ANGLE_GROUPS_ID: AngleGroupDef[] = [
  { id: '1', title: '1 · Brutal Appetite', subtitle: 'Sudut 1–4' },
  { id: '2', title: '2 · Playful Doodle', subtitle: 'Sudut 5–8' },
  { id: '3', title: '3 · Modern Dimension', subtitle: 'Sudut 9–12' },
  { id: '4', title: '4 · Creative Collage', subtitle: 'Sudut 13–16' },
  { id: '5', title: '5 · Elegant Noir', subtitle: 'Sudut 17–20' },
  { id: '6', title: '6 · Organic Origin', subtitle: 'Sudut 21–24' },
  { id: '7', title: '7 · Dynamic Motion', subtitle: 'Sudut 25–28' },
  { id: '8', title: '8 · Graphic Color Block', subtitle: 'Sudut 29–32' },
];
...
const getAngleGroupIndex = (index: number): number => {
  ...
  if (index < PHOTOSHOOT_LABELS.length) {
    return Math.floor(index / 4);
  }

  const extendedStart = PHOTOSHOOT_LABELS.length;
  const extendedEnd = PHOTOSHOOT_LABELS.length + PHOTOSHOOT_EXTENDED_LABELS.length;
  if (index >= extendedStart && index < extendedEnd) {
    return 4 + Math.floor((index - extendedStart) / 4);
  }

  const baseIndex = index % PHOTOSHOOT_LABELS.length;
  return Math.floor(baseIndex / 4);
};

const GeneratePosterPage: React.FC = () => {
  ...
  const getRemainingSecondsForVideo = (video: CharacterAngleVideoOutput | null): number => {
    if (!video || !video.startedAt || !video.estimatedTotalSeconds) return 0;
    const elapsed = Math.floor((now - video.startedAt) / 1000);
    const remaining = Math.max(0, video.estimatedTotalSeconds - elapsed);
    return remaining;
  };

  const getCountdownMessageVideo = (video: CharacterAngleVideoOutput | null): string | null => {
    ...
    const unsubscribe = window.zeoAPI.onBatchUpdate?.((update: any) => {
      ...
      if ((workflow.includes('gem_pix') || workflow.includes('affiliate images')) && category === 'character') {
        if (update.type === 'SCENE_COMPLETED') {
          const sceneIndex: number | null = typeof update.index === 'number' && Number.isFinite(update.index) ? update.index : null;
          if (sceneIndex) {
            ...
            // Clear regenerating state agar card tidak stuck (real-time progressive rendering)
      ...
      // Accept 'product' category for poster video generation
      if (category !== 'product') {
        return;
      }
      ...
      if (update.type === 'SCENE_COMPLETED') {
        ...
        if (sceneIndex && update.fileName && update.filePath) {
          ...
          // Clear video generating state agar tidak stuck
      ...
      if (update.type === 'ERROR' || update.type === 'SCENE_ERROR') {
        ...
        if (sceneIndex) {
          ...
          // Update video status to 'failed' agar tidak stuck di "Generating Video..."
          setAngleVideos((prev) => {
            const next = [...prev];
            if (angleIndex >= 0 && next[angleIndex]) {
              next[angleIndex] = {
        ...
        if (message) {
          if (message.includes('PUBLIC_ERROR_AUDIO_FILTERED')) {
            const friendlyMessage = t.posterGenerator.logVideoFilteredError;
  ...
  const addLog = (type: ActivityLogEntry['type'], message: string) => {
    if (!message) return;
    const prefixedMessage = `[Poster] ${message}`;
  ...
  const runProductAnalysis = async (file: File) => {
    try {
      if (typeof window === 'undefined' || !window.zeoAPI?.analyzeCharacterImage) {
        addLog('INFO', t.logMessages.common.engineNotAvailable);
        return;
      }
      ...
      if (!aiProvider || !apiKey) {
        addLog('INFO', t.logMessages.ai.configIncomplete);
        return;
      }

      setAnalysisSuccess(false);
      setIsAnalyzing(true);
      addLog('INFO', t.logMessages.poster.analysisStarted);
      ...
      if (!result || !result.ok || !result.analysis) {
        const message: string = (result && result.error) || t.logMessages.poster.analysisFailed;
        addLog('ERROR', message);
        setError(message);
        setAnalysisSuccess(false);
        return;
      ...
      addLog('SUCCESS', t.logMessages.poster.analysisSuccess);
      setAnalysisSuccess(true);
    } catch (err: any) {
      const message = err?.message || t.logMessages.poster.analysisFailed;
      addLog('ERROR', message);
      setError(message);
      setAnalysisSuccess(false);
    } finally {
  ...
  const handleRegenerateImage = async (index: number, options?: { bypassGuard?: boolean }) => {
    ...
    if (!bearerKey) {
      ...
      addLog('ERROR', t.posterGenerator.logRegenBearerMissing.replace('{message}', bearerTokenMissingMessage));
      return;
    }

    if (typeof window === 'undefined' || !window.zeoAPI?.generateAffiliateImages) {
      setError(engineNotAvailableMessage);
      addLog('ERROR', t.posterGenerator.logRegenEngineNotAvailable.replace('{message}', engineNotAvailableMessage));
      return;
    }

    const mainPrompt = buildCharacterPrompt(formData);
    const anglePrompts = PHOTOSHOOT_DESCRIPTIONS;

    const angleIdx = index;
    const baseAngle = anglePrompts[angleIdx] || anglePrompts[0];
    const label = PHOTOSHOOT_LABELS[angleIdx] || `professional angle ${angleIdx + 1}`;
    ...
    try {
      ...
      if (!response || !response.ok) {
        const message = response?.error || t.posterGenerator.logEngineRegenInvalid;
      ...
      if (!result || !result.success || !result.dataUrl) {
        const errMsg: string = result?.error || t.posterGenerator.logEngineRegenFailed;
        setError(errMsg);
        addLog('ERROR', t.posterGenerator.logRegenFailed.replace('{label}', label).replace('{error}', errMsg));
        return;
      }
      ...
      const message = err?.message || t.posterGenerator.logRegenError;
      setError(message);
      addLog('ERROR', message);
    } finally {
  ...
  const handleDownloadAll = async () => {
    ...
    addLog('INFO', t.logMessages.poster.zipPreparing);
    ...
    for (const { src, index } of downloadTargets) {
      ...
      try {
        ...
        const perBatch = PHOTOSHOOT_LABELS.length;
        ...
        const filename = `poster-${safeLabel}${setSuffix}.png`;
    ...
    const link = document.createElement('a');
    link.href = zipUrl;
    link.setAttribute('download', 'poster-gallery.zip');
    document.body.appendChild(link);
    link.click();
    link.parentNode?.removeChild(link);

    URL.revokeObjectURL(zipUrl);
    addLog('SUCCESS', t.logMessages.poster.zipReady);
  };
  ...
  const handleRegenerateFailed = async () => {
    ...
    try {
      for (const idx of failedIndexes) {
        // Regenerate each failed angle one by one using the existing handler while bypassing the global loading guard.
        // eslint-disable-next-line no-await-in-loop
        await handleRegenerateImage(idx, { bypassGuard: true });
      }
    } finally {
      setIsLoading(false);
      setRegeneratingIndex(null);
  ...
  const handleOpenCharacterEditModal = (index: number) => {
    ...
    if (!src.startsWith('data:image')) {
      addLog('ERROR', t.posterGenerator.logEditOpenInvalidFormat.replace('{index}', String(index + 1)));
      return;
    }
  ...
  const handleApplyCharacterEdit = async () => {
    ...
    if (!imageUrl.startsWith('data:image')) {
      addLog('ERROR', t.posterGenerator.logEditRunInvalidFormat.replace('{index}', String(index + 1)));
      return;
    }
    ...
    if (typeof window === 'undefined' || !window.zeoAPI?.editStoryFrame) {
      addLog(
        'ERROR',
        t.posterGenerator.logEditEngineNotAvailable.replace('{index}', String(index + 1)),
      );
      return;
    }
    ...
    if (!bearerKey.trim()) {
      ...
      addLog('ERROR', t.posterGenerator.logEditBearerMissing.replace('{index}', String(index + 1)).replace('{message}', message));
      setError(message);
      return;
    }
    ...
    addLog('INFO', t.posterGenerator.logEditProcessing.replace('{label}', label));
    ...
    try {
      ...
      if (!result || !result.ok || !result.dataUrl) {
        const message = (result && result.error) || t.posterGenerator.logNewImageFailed;
        addLog('ERROR', t.posterGenerator.logEditFailed.replace('{label}', label).replace('{error}', message));
      ...
      setSelectedPreviewImage(newUrl);
      setLightboxImage(newUrl);
      addLog('SUCCESS', t.posterGenerator.logEditSuccess.replace('{label}', label));
      setCharacterEditModal((prev) => ({ ...prev, isSubmitting: false }));
    } catch (err: any) {
  ...
  const handleExtendGenerate = async () => {
    ...
    if (!bearerKey) {
      setError(bearerTokenMissingMessage);
      addLog('ERROR', t.posterGenerator.logExtendBearerMissing);
      return;
    }

    if (typeof window === 'undefined' || !window.zeoAPI?.generateAffiliateImages) {
      setError(engineNotAvailableMessage);
      addLog('ERROR', t.posterGenerator.logExtendEngineNotAvailable);
      return;
    }

    const mainPrompt = buildCharacterPrompt(formData);

    const totalExtendedAngles = POSTER_EXTENDED_DESCRIPTIONS.length;
    ...
    if (nextExtendedCharacterAngleIndex >= totalExtendedAngles) {
      addLog('INFO', t.logMessages.poster.extendNoMoreBatches);
      return;
    }
    ...
    addLog(
      'INFO',
      isFirstExtendBatch
        ? t.posterGenerator.logExtendStarting
        : t.posterGenerator.logExtendContinuing.replace('{batch}', String(currentExtendBatchIndexLocal)).replace('{total}', String(totalExtendedBatches)),
    );

    try {
      const aspectRatioKey = mapAspectRatioToEngineKey(aspectRatio);

      const anglePrompts = POSTER_EXTENDED_DESCRIPTIONS;
      ...
      while (cursor < totalExtendedAngles) {
        ...
        batchTargets = anglePrompts
          ...
          .map(({ localIndex, angleText }) => ({
            localIndex,
            globalIndex: PHOTOSHOOT_LABELS.length + localIndex,
            angleText,
          }))
        ...
        batchTargets.forEach(({ localIndex, angleText }) => {
          const labelBase =
            PHOTOSHOOT_EXTENDED_LABELS[localIndex] || `Extended angle ${localIndex + 1}`;
          ...
          addLog('INFO', t.posterGenerator.logExtendPreparing.replace('{label}', labelBase));
        });
        ...
        addLog(
          'INFO',
          t.posterGenerator.logExtendSkipped.replace('{batch}', String(batchIndexDisplay)).replace('{total}', String(totalExtendedBatches)).replace('{group}', groupTitle),
        );
        cursor = batchEnd;
      }

      if (items.length === 0) {
        addLog('INFO', t.logMessages.poster.noActiveExtendedLabels);
        return;
      }

      {
        ...
        addLog(
          'INFO',
          t.posterGenerator.logExtendRunning.replace('{batch}', String(batchIndexDisplay)).replace('{total}', String(totalExtendedBatches)).replace('{group}', groupTitle),
        );
      }

      // Create placeholders with status tracking
      setAngleImages((prev) => {
        const next = [...prev];
        const globalBatchEnd = PHOTOSHOOT_LABELS.length + batchEnd;
        const oldLength = next.length;
        ...
        batchTargets.forEach(({ globalIndex }) => {
          const localIdx = globalIndex - PHOTOSHOOT_LABELS.length;
          const label = PHOTOSHOOT_EXTENDED_LABELS[localIdx] || `Extended angle ${localIdx + 1}`;
          next[globalIndex] = {
            dataUrl: '',
            status: 'generating' as const,
            startedAt: Date.now(),
      ...
      batchTargets.forEach((target, idx) => {
        const cardId = `poster-image-${target.globalIndex}`;
        const timeout = setTimeout(() => {
      ...
      addLog(
        'INFO',
        t.posterGenerator.logExtendSending.replace('{count}', String(items.length)).replace('{ratio}', aspectRatio).replace('{batch}', String(batchIndexDisplay)).replace('{total}', String(totalExtendedBatches)),
      );
      ...
      if (!response || !response.ok) {
        const message = response?.error || t.posterGenerator.logExtendEngineInvalid;
        throw new Error(message);
      }

      const results: any[] = Array.isArray(response.results) ? response.results : [];
      if (!results.length) {
        throw new Error(t.posterGenerator.logExtendNoResults);
      }
      ...
      // Update images with results (batch processing after API call)
      setAngleImages((prev) => {
        const next = [...prev];
        ...
        for (let i = 0; i < Math.min(batchReturned, batchRequested); i += 1) {
          ...
          const globalIndex = target?.globalIndex ?? PHOTOSHOOT_LABELS.length + localIndex;
          const labelBase = PHOTOSHOOT_EXTENDED_LABELS[localIndex] || `Extended angle ${localIndex + 1}`;
          ...
          if (r && r.success && r.dataUrl) {
            ...
            const errMsg: string =
              ...
              t.posterGenerator.logEngineFailedLabel;
            next[globalIndex] = {
              dataUrl: '',
              status: 'failed' as const,
              startedAt: existingImage?.startedAt,
            ...
            addLog('ERROR', t.posterGenerator.logExtendFailed.replace('{label}', label).replace('{error}', errMsg));
          }
        }
      ...
      setAngleImages((prev) => {
        const firstBatchIndex = prev.findIndex((img, idx) => {
          const localIdx = idx - PHOTOSHOOT_LABELS.length;
        ...
        if (firstBatchIndex >= 0) {
          setSelectedPreviewImage(prev[firstBatchIndex]?.dataUrl || null);
        }
        return prev;
      });
      ...
      if (successCount > 0) {
        addLog(
          'SUCCESS',
          t.posterGenerator.logExtendBatchComplete.replace('{batch}', String(batchIndexDisplay)).replace('{total}', String(totalExtendedBatches)).replace('{success}', String(successCount)).replace('{requested}', String(batchRequested)),
        );
        addLog('INFO', t.posterGenerator.logExtendBatchAngles.replace('{labels}', successLabels.join('; ')));
      }

      if (failedWithErrorCount > 0 || missingCount > 0) {
        addLog(
          'INFO',
          t.posterGenerator.logExtendBatchSummary.replace('{batch}', String(batchIndexDisplay)).replace('{requested}', String(batchRequested)).replace('{returned}', String(batchReturned)).replace('{success}', String(successCount)).replace('{failed}', String(failedWithErrorCount)).replace('{missing}', String(missingCount)),
        );
      }

      setNextExtendedCharacterAngleIndex(batchEnd);
    } catch (err: any) {
      const message = err?.message || t.posterGenerator.logExtendError;
      setError(message);
      addLog('ERROR', message);
    } finally {
      setIsLoading(false);
  ...
  const handleGenerate = useCallback(async () => {
    ...
    if (!bearerKey) {
      setError(bearerTokenMissingMessage);
      addLog('ERROR', t.posterGenerator.logGenerateBearerMissing);
      return;
    }

    if (typeof window === 'undefined' || !window.zeoAPI?.generateAffiliateImages) {
      setError(engineNotAvailableMessage);
      addLog('ERROR', t.posterGenerator.logGenerateEngineNotAvailable);
      return;
    }

    const mainPrompt = buildProductPrompt(formData, aspectRatio, !!uploadedImage);

    const totalAngles = PHOTOSHOOT_DESCRIPTIONS.length;
    ...
    if (isFirstBatch) {
      ...
      addLog('INFO', t.logMessages.poster.generateStarted);
    }

    addLog(
      'INFO',
      isFirstBatch
        ? t.posterGenerator.logGenerateDetermining
        : t.posterGenerator.logGenerateContinuing.replace('{batch}', String(currentBatchIndex)).replace('{total}', String(totalBatches)),
    );
    ...
    try {
      ...
      while (cursor < totalAngles) {
        ...
        batchTargets.forEach(({ globalIndex, angleText }) => {
          const label = PHOTOSHOOT_LABELS[globalIndex] || `Poster shot ${globalIndex + 1}`;
          const fullPrompt = `${mainPrompt} Poster concept ${
            globalIndex + 1
          ...
          addLog('INFO', t.posterGenerator.logGeneratePreparing.replace('{label}', label));
        });
        ...
        addLog(
          'INFO',
          t.posterGenerator.logGenerateSkipped.replace('{batch}', String(batchIndexDisplay)).replace('{total}', String(totalBatches)).replace('{group}', groupTitle),
        );
        cursor = batchEnd;
      }

      if (items.length === 0) {
        addLog('INFO', t.logMessages.poster.noActiveMainLabels);
        return;
      }

      {
        ...
        addLog(
          'INFO',
          t.posterGenerator.logGenerateRunning.replace('{batch}', String(batchIndexDisplay)).replace('{total}', String(totalBatches)).replace('{group}', groupTitle),
        );
      }
      ...
      if (items.length === 0) {
        addLog(
          'INFO',
          t.posterGenerator.logGenerateAllSkipped.replace('{batch}', String(batchIndexDisplay)).replace('{total}', String(totalBatches)),
        );
      ...
      addLog(
        'INFO',
        t.posterGenerator.logGenerateSending.replace('{count}', String(items.length)).replace('{ratio}', aspectRatio).replace('{batch}', String(batchIndexDisplay)).replace('{total}', String(totalBatches)),
      );
      ...
      if (!response || !response.ok) {
        const message = response?.error || t.posterGenerator.logGenerateEngineInvalid;
        throw new Error(message);
      }

      const results: any[] = Array.isArray(response.results) ? response.results : [];
      if (!results.length) {
        throw new Error(t.posterGenerator.logGenerateNoResults);
      }
      ...
      setAngleImages((prev) => {
        ...
        for (let i = 0; i < Math.min(batchReturned, batchRequested); i += 1) {
          ...
          if (r && r.success && r.dataUrl) {
            ...
            addLog('ERROR', t.posterGenerator.logEngineFailed.replace('{label}', label).replace('{error}', errMsg));
          }
        }
      ...
      setAngleImages((prev) => {
        ...
        if (firstBatchIndex >= 0) {
          ...
          if (firstAvailableIndex >= 0) {
            setSelectedPreviewImage(prev[firstAvailableIndex]?.dataUrl || null);
          }
        }
        return prev;
      });

      const totalSuccessSoFar = angleImages.filter((img) => img?.status === 'completed' && !!img.dataUrl).length;

      if (successCount > 0) {
        addLog(
          'SUCCESS',
          t.posterGenerator.logGenerateBatchComplete.replace('{batch}', String(batchIndexDisplay)).replace('{total}', String(totalBatches)).replace('{success}', String(successCount)).replace('{requested}', String(batchRequested)).replace('{totalSuccess}', String(totalSuccessSoFar)).replace('{totalAngles}', String(totalAngles)),
        );
        addLog('INFO', t.posterGenerator.logGenerateBatchAngles.replace('{labels}', successLabels.join('; ')));
      }

      if (failedWithErrorCount > 0 || missingCount > 0) {
        addLog(
          'INFO',
          t.posterGenerator.logGenerateBatchSummary.replace('{batch}', String(batchIndexDisplay)).replace('{requested}', String(batchRequested)).replace('{returned}', String(batchReturned)).replace('{success}', String(successCount)).replace('{failed}', String(failedWithErrorCount)).replace('{missing}', String(missingCount)),
        );
      }

      setNextCharacterAngleIndex(batchEnd);
    } catch (err: any) {
      const message = err?.message || t.posterGenerator.logGenerateError;
      setError(message);
      addLog('ERROR', message);
    } finally {
      setIsLoading(false);
  ...
  const totalEnabledAngleGroups = enabledAngleGroups.filter(Boolean).length;
  ...
  const handleGenerateByChecklist = async () => {
    ...
    addLog('INFO', t.logMessages.poster.noActiveLabels);
  };
  ...
  const visibleAngleEntries = allPreviewImages
    ...
    .filter(({ img, index }) => {
      ...
      return isAngleEnabled(index) || (img && (img.status === 'completed' || img.status === 'generating')) || hasVideo;
    });

  const totalMainAngles = PHOTOSHOOT_LABELS.length;
  const enabledMainAnglesCount = PHOTOSHOOT_LABELS.filter((_, idx) => isAngleEnabled(idx)).length;
  ...
  const totalEnabledExtendedBatches = (() => {
    const offset = PHOTOSHOOT_LABELS.length;
  ...
  const hasAnyExtendedImage = angleImages.some(
    (src, idx) => idx >= PHOTOSHOOT_LABELS.length && !!src,
  );
  ...
  const hasFailedAngles = angleImages.some((img, idx) => isAngleEnabled(idx) && (!img || img.status === 'failed' || (img.status !== 'generating' && !img.dataUrl)));
  ...
  const getAngleLabelByIndex = (index: number): string => {
    if (index < 0) return t.posterGenerator.noPhotoYet;
    const perBatch = PHOTOSHOOT_LABELS.length;
    const batchIndex = Math.floor(index / perBatch);
    const baseIndex = index % perBatch;

    if (batchIndex === 0) {
      return PHOTOSHOOT_LABELS[baseIndex] || `Professional angle ${baseIndex + 1}`;
    }

    if (batchIndex === 1) {
      return PHOTOSHOOT_EXTENDED_LABELS[baseIndex] || `Extended angle ${baseIndex + 1}`;
    }

    const fallbackBase =
      PHOTOSHOOT_EXTENDED_LABELS[baseIndex] || PHOTOSHOOT_LABELS[baseIndex] || `Professional angle ${baseIndex + 1}`;
  ...
  const buildCharacterVideoPromptForAngle = (index: number): string => {
    const label = getAngleLabelByIndex(index) || `Poster variation ${index + 1}`;
    ...
    parts.push('[Affiliate Video - Product/Poster]');
    parts.push(
      'MANDATORY: For the ENTIRE 6–8 second video, the VEO engine must keep the product and poster design 100% identical to the reference and provided image in EVERY frame: product appearance, colors, text, graphics, layout, and branding must not change, morph, or transform mid-video.',
    );
    parts.push(
      'Video must be a single continuous shot with no cuts or obvious transitions. Motion is limited to subtle camera moves (slow zoom, gentle pan, slight rotation) and natural environmental effects (light shifts, ambient motion). The product/poster itself remains static and unchanged. ABSOLUTELY NO additional text, watermark, logo overlay, UI elements, subtitles, or captions beyond what exists in the original poster design.',
    );
    parts.push(`Variation/angle: ${label}.`);
  ...
  const handleGenerateAngleVideo = async (index: number, overridePrompt?: string) => {
    if (!isAngleEnabled(index)) {
      addLog('INFO', t.posterGenerator.logVideoAngleDisabled.replace('{label}', getAngleLabelByIndex(index)));
      return;
    }
    ...
    if (!src) {
      addLog('ERROR', t.posterGenerator.logVideoNoPhoto.replace('{label}', label));
      return;
    }
    ...
    if (!src.startsWith('data:image')) {
      addLog(
        'ERROR',
        t.posterGenerator.logVideoInvalidFormat.replace('{label}', label),
      );
      return;
    }
    ...
    if (parts.length < 2 || !parts[1].trim()) {
      addLog(
        'ERROR',
        t.posterGenerator.logVideoInvalidData.replace('{label}', label),
      );
      return;
    }

    if (typeof window === 'undefined' || !window.zeoAPI?.startAffiliateVideoWorkflow) {
      const message = t.posterGenerator.logVideoEngineNotAvailable;
      setError(message);
      addLog('ERROR', message);
      return;
    }
    ...
    if (!bearerKey.trim()) {
      const message = t.posterGenerator.logVideoBearerMissing;
      setError(message);
      addLog('ERROR', message);
      return;
    }
    ...
    if (!downloadPath.trim()) {
      const message = t.posterGenerator.logVideoOutputMissing;
      setError(message);
      addLog('ERROR', message);
      return;
    }
    ...
    try {
      ...
      addLog('INFO', t.posterGenerator.logVideoStarting.replace('{label}', label));
      ...
      const message = err?.message || t.posterGenerator.logVideoError;
      setError(message);
      addLog('ERROR', message);
  ...
  const handleGenerateAllVideos = async () => {
    if (isBatchVideoRunning || anyLoading || videoGeneratingIndexes.length > 0) {
      addLog(
        'ERROR',
        t.posterGenerator.logConvertAllBusy,
      );
      return;
    }
    ...
    if (!targets.length) {
      addLog(
        'INFO',
        t.posterGenerator.logConvertAllNoTargets,
      );
      return;
    }
    ...
    addLog(
      'INFO',
      t.posterGenerator.logConvertAllStarting.replace('{count}', String(targets.length)).replace('{max}', String(MAX_PARALLEL_VIDEO)),
    );
    ...
    try {
      ...
      addLog(
        'SUCCESS',
        t.posterGenerator.logConvertAllComplete,
      );
    } catch (err: any) {
      const message =
        err?.message ||
        t.posterGenerator.logConvertAllError;
      setError(message);
      addLog('ERROR', message);
    } finally {
      setIsBatchVideoRunning(false);
  ...
  return (
    <div className="h-full w-full flex flex-col min-h-0 text-gray-50">
      <PageHeader
        iconId="generate-poster"
        iconClassName="h-6 w-6 mr-3 text-white"
        title={t.posterGenerator.title}
        description={t.posterGenerator.description}
        tutorialUrl={POSTER_TUTORIAL_URL}
        tutorialTitle="Tutorial Generate Poster"
        tutorialMode="direct"
      />
        ...
        <div className="flex h-full min-w-0 gap-4">
          ...
          >
            <div className="flex-1 flex flex-col overflow-y-auto custom-scrollbar px-6 py-4 space-y-4">
              <div className="p-4 rounded-lg bg-zinc-900/80 border border-zinc-800">
                <h2 className="text-sm font-semibold text-gray-100 mb-3">
                  {t.posterGenerator.uploadProductPhoto}
                </h2>
                <p className="text-xs text-gray-400 mb-4">
                  {t.posterGenerator.uploadProductDesc}
                </p>
                <div className="flex flex-col sm:flex-row gap-4 items-center">
                  <label className="w-28 h-28 bg-zinc-950 rounded-lg border-2 border-dashed border-zinc-700 flex items-center justify-center text-center text-gray-400 text-[11px] cursor-pointer hover:border-purple-500 transition shrink-0">
                    {uploadedImage ? (
                      <img
                        ...
                        alt={t.posterGenerator.altProductReference}
                        className="w-full h-full object-cover rounded-md"
                      />
                    ) : (
                      <span>{t.posterGenerator.clickToUpload}</span>
                    )}
                    <input
                      type="file"
                  ...
                  <div className="flex-grow flex flex-col gap-2 w-full">
                    {uploadedImage && (
                      ...
                      >
                        {t.posterGenerator.deleteProductPhoto}
                      </button>
                    )}
                    <div className="text-[11px] text-gray-400">
                      {t.posterGenerator.noPhotoWarning}
                    </div>
                    {isAnalyzing && (
                      <div className="mt-1 text-[11px] text-purple-300 flex items-center gap-1">
                        <span className="w-2 h-2 rounded-full bg-purple-400 animate-pulse" />
                        <span>{t.posterGenerator.analyzingProduct}</span>
                      </div>
                    )}
                    {!isAnalyzing && analysisSuccess && (
                      <div className="mt-1 text-[11px] text-emerald-300 flex flex-col gap-1">
                        <div className="flex items-center gap-1">
                          ...
                          <span>{t.posterGenerator.analysisSuccessMsg}</span>
                        </div>
                      </div>
                    )}
                  </div>
              ...
              <div className="p-4 rounded-lg bg-zinc-900/80 border border-zinc-800 space-y-4">
                <h2 className="text-sm font-semibold text-gray-100">{t.posterGenerator.productIdentity}</h2>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  <LabeledInput
                    label={t.posterGenerator.labelProductType}
                    name="productType"
                    value={formData.productType}
                    onChange={handleFormChange as any}
                  />
                  <LabeledInput
                    label={t.posterGenerator.labelBrandStyle}
                    name="brandStyle"
                    value={formData.brandStyle}
                    onChange={handleFormChange as any}
                  />
                  <LabeledInput
                    label={t.posterGenerator.labelMainMaterial}
                    name="mainMaterial"
                    value={formData.mainMaterial}
                    onChange={handleFormChange as any}
                  />
                  <LabeledInput
                    label={t.posterGenerator.labelColor}
                    name="color"
                    value={formData.color}
                    onChange={handleFormChange as any}
                  />
                  <LabeledInput
                    label={t.posterGenerator.labelUniqueDetails}
                    name="uniqueDetails"
              ...
              <div className="p-4 rounded-lg bg-zinc-900/80 border border-zinc-800 space-y-4">
                <h2 className="text-sm font-semibold text-gray-100">{t.posterGenerator.compositionVisual}</h2>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  <LabeledInput
                    label={t.posterGenerator.labelAngle}
                    name="angle"
                    value={formData.angle}
                    onChange={handleFormChange as any}
                  />
                  <LabeledInput
                    label={t.posterGenerator.labelFocus}
                    name="focus"
                    value={formData.focus}
                    onChange={handleFormChange as any}
                  />
                  <LabeledInput
                    label={t.posterGenerator.labelActionState}
                    name="actionState"
              ...
              <div className="p-4 rounded-lg bg-zinc-900/80 border border-zinc-800 space-y-4">
                <h2 className="text-sm font-semibold text-gray-100">{t.posterGenerator.environmentLighting}</h2>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  <LabeledInput
                    label={t.posterGenerator.labelBackground}
                    name="background"
                    value={formData.background}
                    onChange={handleFormChange as any}
                  />
                  <LabeledInput
                    label={t.posterGenerator.labelSecondaryElements}
                    name="secondaryElements"
                    value={formData.secondaryElements}
                    onChange={handleFormChange as any}
                  />
                  <LabeledInput
                    label={t.posterGenerator.labelMood}
                    name="mood"
                    value={formData.mood}
                    onChange={handleFormChange as any}
                  />
                  <LabeledInput
                    label={t.posterGenerator.labelLighting}
                    name="lighting"
                    value={formData.lighting}
                    onChange={handleFormChange as any}
                  />
                  <LabeledInput
                    label={t.posterGenerator.labelShadows}
                    name="shadows"
                    value={formData.shadows}
                    onChange={handleFormChange as any}
                  />
                  <LabeledInput
                    label={t.posterGenerator.labelPhotoStyle}
                    name="photoStyle"
              ...
              <div className="p-4 rounded-lg bg-zinc-900/80 border border-zinc-800 space-y-4">
                <div>
                  <span className="block text-xs font-semibold text-gray-300 mb-2">{t.posterGenerator.aspectRatioLabel}</span>
                ...
                <div>
                  <div className="flex items-center justify-between mb-2">
                    <span className="block text-xs font-semibold text-gray-300">{t.posterGenerator.posterVariationsTitle}</span>
                  ...
                  <p className="text-[11px] text-gray-500 mb-2">
                    {t.posterGenerator.posterVariationsDesc}
                  </p>
            ...
            <div className="px-6 pb-5 pt-3 border-t border-zinc-800 space-y-3">
              ...
              >
                {isLoading
                  ? t.posterGenerator.generatingProcess
                  : authReady
                  ? t.posterGenerator.generatePosterButton
                  : t.posterGenerator.testTokenFirst}
              </button>
          ...
          >
            <div className="px-6 py-4 border-b border-zinc-800 flex items-center justify-between">
              <h3 className="text-lg font-semibold text-gray-50">{t.posterGenerator.previewPoster}</h3>
              <button
                type="button"
              ...
              >
                ...
                <span>{t.posterGenerator.clearData}</span>
              </button>
            </div>
            ...
            <div className="flex-1 flex flex-col px-6 py-4 min-h-0 overflow-y-auto custom-scrollbar">
              {anyLoading && !hasGeneratedOnce && (
                <div className="flex-1 flex items-center justify-center">
                  <GradientLoader 
                    size="md" 
                    text={t.posterGenerator.generatingProcess}
              ...
              {!anyLoading && !hasGeneratedOnce && (
                <div className="flex-1 flex items-center justify-center bg-zinc-950/40 rounded-lg text-gray-500 text-center text-xs p-4">
                  <p>
                    {t.posterGenerator.previewHint}
                    {' '}
                    <span className="font-semibold text-gray-300">{t.posterGenerator.generatePosterButton}</span>.
                  </p>
                </div>
              )}
              ...
              {!isLoading && hasGeneratedOnce && (
                <div className="mb-3 text-[11px] text-gray-300">
                  <span className="font-semibold text-gray-100">
                    {t.posterGenerator.previewStatsTotal.replace('{count}', String(successfulImageCount))}
                  </span>
                  <span className="mx-1 text-gray-500">·</span>
                  <span className="text-gray-300">
                    {t.posterGenerator.previewStatsShots.replace('{slots}', String(visibleAngleEntries.length)).replace('{batch}', String(Math.ceil(visibleAngleEntries.length / PHOTOSHOOT_LABELS.length))).replace('{label}', activeLabel)}
                  </span>
                  <p className="text-gray-500 mt-0.5">
                    {t.posterGenerator.gridInstruction}
                  </p>
                </div>
              )}
              ...
              {hasGeneratedOnce && (
                <div className="pt-3 border-t border-zinc-800 space-y-2">
                  <div className="flex items-center justify-between">
                    <div>
                      <h4 className="text-xs font-semibold text-gray-100">{t.posterGenerator.photoshootTitle}</h4>
                      <p className="text-[10px] text-gray-500">
                        {t.posterGenerator.photoshootDesc}
                      </p>
                    </div>
                    <div className="flex items-center gap-2">
                      <button
                      ...
                      >
                        ...
                        <span className="whitespace-nowrap">
                          {isBatchVideoRunning ? t.posterGenerator.processingStatus : t.posterGenerator.convertAllToVideo}
                        </span>
                      </button>
                      {canGenerateNextMainBatch && (
                        ...
                        >
                          <span className="whitespace-nowrap">
                            {anyLoading
                              ? t.posterGenerator.processingStatus
                              : t.posterGenerator.next4ShotsBatch.replace('{batch}', String(nextMainBatchIndex)).replace('{total}', String(totalMainBatches))}
                          </span>
                        </button>
                      )}
                      {hasMoreThanOneLabel &&
                          ...
                          >
                            ...
                            <span className="whitespace-nowrap">
                              {isLoading
                                ? t.posterGenerator.processingStatus
                                : hasAnyExtendedImage
                                ? t.posterGenerator.next4ExtendedShots.replace('{batch}', String(currentExtendBatchIndexUiGlobal)).replace('{total}', String(totalEnabledAngleBatchesUi))
                                : `${t.posterGenerator.extendGenerate} (Batch 1/${totalEnabledAngleBatchesUi})`}
                            </span>
                          </button>
                        )}
                      <button
                      ...
                      >
                        ...
                        <span className="whitespace-nowrap">
                          {isRegeneratingFailed ? t.posterGenerator.processingStatus : t.posterGenerator.regenerateFailed}
                        </span>
                      </button>
                      <button
                        type="button"
                      ...
                      >
                        ...
                        <span className="whitespace-nowrap">{t.posterGenerator.downloadAllImages}</span>
                      </button>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                    {visibleAngleEntries.map(({ img, index }) => {
                      ...
                      const cardId = `poster-image-${index}`;
                      const isVisible = visibleCardIds.has(cardId);

                      const isActive = imageOutput?.status === 'completed' && !!src && mainPreviewImage === src;
                      ...
                      const isVideoGenerating = videoOutput?.status === 'generating';
                      const isVideoFailed = videoOutput?.status === 'failed';
                      const countdownMsgVideo = getCountdownMessageVideo(videoOutput);
                      const videoUiState = getVideoUiState(index);
                      const viewMode: CharacterAngleViewMode =
                        angleViewModes[index] || (videoOutput && (videoOutput.status === 'completed' || videoOutput.dataUrl) ? 'video' : 'photo');
                      const isBusy = isCardRegenerating || isCardEditing || isVideoGenerating;
                      ...
                      return (
                        <div
                          key={index}
                          className={`relative group ${thumbnailAspectClass} cursor-pointer rounded-md overflow-hidden 
                            ...
                            <div className="w-full h-full relative flex flex-col items-center justify-center text-gray-100">
                              ...
                              <div className="relative z-10 flex flex-col items-center text-center px-3">
                                ...
                                <div className="mt-1 text-[10px] text-gray-200 px-2 text-center">
                                  {t.posterGenerator.generateVideoStatus || 'Generating Video...'}
                                </div>
                              </div>
                            </div>
                          ) : isVideoFailed && viewMode === 'video' ? (
                            <div className="w-full h-full flex flex-col items-center justify-center bg-red-950/40 text-red-300">
                              <div className="text-2xl mb-2">⚠️</div>
                              <div className="text-xs font-semibold">Video Failed</div>
                              {videoOutput?.errorMessage && (
                                <div className="mt-1 text-[10px] text-red-400 px-2 text-center line-clamp-3">
                                  {videoOutput.errorMessage.includes('PUBLIC_ERROR_AUDIO_FILTERED') 
                              ...
                              <button
                                ...
                                className="mt-2 px-2.5 py-1 rounded-lg bg-red-600/20 border border-red-500/30 text-red-200 hover:bg-red-600/30 text-[10px] font-semibold transition"
                              >
                                Lihat Foto
                              </button>
                            </div>
                          ...
                          {isCardEditing && isImageGenerating && (
                            <div className="absolute inset-0 flex flex-col items-center justify-center z-30">
                              ...
                              <div className="relative z-10 flex flex-col items-center text-center px-3 text-gray-100">
                                ...
                                <div className="mt-1 text-[10px] text-gray-200 px-2 text-center">
                                  {t.conceptGenerator?.editingStatus || 'Editing...'}
                                </div>
                              </div>
                            </div>
                          )}
                          ...
                          {isCardRegenerating && isImageGenerating && (
                            <div className="absolute inset-0 flex flex-col items-center justify-center z-30">
                              ...
                              <div className="relative z-10 flex flex-col items-center text-center px-3 text-gray-100">
                                ...
                                <div className="mt-1 text-[10px] text-gray-200 px-2 text-center">
                                  {t.conceptGenerator?.regenStatus || 'Regenerasi...'}
                                </div>
                              </div>
                            </div>
                          )}

                          {videoOutput && (videoOutput.status === 'completed' || videoOutput.dataUrl) && viewMode === 'photo' && !isVideoGenerating && (
                            <div className="absolute bottom-1 left-1 right-1 px-2 py-1 rounded-md bg-green-900/30 border border-green-700/50 text-center">
                                  ...
                                  <div className="inline-flex flex-col items-stretch gap-1">
                                    <div className="flex items-center justify-center gap-1">
                                      <button
                                        ...
                                        title={t.posterGenerator.editBtn}
                                      >
                                        {t.posterGenerator.editBtn}
                                      </button>
                                      <button
                                        type="button"
                                        onClick={(event) => {
                                        ...
                                        disabled={!authReady}
                                        className="px-2.5 py-1 rounded-lg text-white text-[11px] font-semibold btn-glass-primary bg-gradient-to-r from-purple-600 to-blue-600 hover:from-purple-700 hover:to-blue-700 transition"
                                        title={t.posterGenerator.regenerateBtn}
                                      >
                                        {t.posterGenerator.regenerateBtn}
                                      </button>
                                      <button
                                        type="button"
                                        onClick={(event) => {
                                        ...
                                        title={t.posterGenerator.downloadFoto}
                                      >
                                        <svg
                                          xmlns="http://www.w3.org/2000/svg"
                                          className="h-3.5 w-3.5"
                                        ...
                                        <span>{t.posterGenerator.downloadFoto}</span>
                                      </button>
                                    </div>
                                    ...
                                    {!videoOutput && (
                                      <div className="flex items-center justify-center gap-1">
                                        <button
                                          ...
                                          title={t.posterGenerator.generateVideoBtn}
                                        >
                                          {t.posterGenerator.generateVideoBtn}
                                        </button>
                                      </div>
                                    )}
                                  </div>
                                ...
                                <div className="w-full flex flex-col items-center gap-1">
                                  <div className="w-full rounded-lg bg-black/70 border border-zinc-700/70 px-2 py-1.5 flex items-center gap-1.5 pointer-events-auto">
                                    <button
                                      ...
                                      title={videoUiState.isPlaying ? t.posterGenerator.pauseTitle : t.posterGenerator.playTitle}
                                    >
                                    ...
                                    <input
                                      ...
                                      title={t.posterGenerator.seekTitle}
                                    />
                                    ...
                                    <button
                                      ...
                                      className="h-6 w-6 rounded-md bg-zinc-900/70 hover:bg-zinc-900 text-white flex items-center justify-center transition"
                                      title={videoUiState.muted ? t.posterGenerator.unmuteTitle : t.posterGenerator.muteTitle}
                                    >
                                    ...
                                    <input
                                      ...
                                      title={t.posterGenerator.volumeTitle}
                                    />
                                  </div>
                                  ...
                                  <div className="flex flex-col items-center gap-1 w-full">
                                    <button
                                      ...
                                      title={t.posterGenerator.regenerateVideoBtn}
                                    >
                                      {t.posterGenerator.regenerateVideoBtn}
                                    </button>
        ...
        <div className="fixed inset-0 z-50 bg-black/80 flex items-center justify-center px-4">
          <div className="bg-zinc-900 border border-zinc-700 rounded-2xl w-full max-w-xl max-h-[90vh] shadow-2xl flex flex-col">
            <div className="flex items-center justify-between px-4 py-3 border-b border-zinc-800">
              <h3 className="text-sm font-semibold text-gray-100">{t.posterGenerator.editPosterTitle}</h3>
              <button
                type="button"
            ...
            <div className="flex-1 p-4 space-y-4 overflow-y-auto custom-scrollbar">
              <div className="w-full bg-zinc-950 rounded-lg overflow-hidden flex items-center justify-center">
                <img
                  ...
                  alt={t.posterGenerator.altPreviewEdit}
                  className="max-h-[60vh] w-auto max-w-full object-contain"
                />
              </div>
              <div className="space-y-2">
                <div className="text-[11px] font-semibold text-gray-200">{t.posterGenerator.editPosterInstruction}</div>
                <textarea
                  ...
                  placeholder={t.posterGenerator.editPosterPlaceholder}
            ...
            <div className="px-4 pb-4 pt-2">
              ...
              >
                {characterEditModal.isSubmitting ? t.posterGenerator.editPosterProcessing : t.posterGenerator.editPosterApply}
              </button>
            </div>
          </div>
        </div>
          ...
          <div className="bg-zinc-900 border border-zinc-700 rounded-2xl w-full max-w-2xl max-h-[90vh] shadow-2xl flex flex-col">
            <div className="flex items-center justify-between px-4 py-3 border-b border-zinc-800">
              <div className="flex flex-col">
                <h3 className="text-sm font-semibold text-gray-100">{t.posterGenerator.videoPromptTitle}</h3>
                <p className="text-[11px] text-gray-500">
            ...
            <div className="flex-1 p-4 space-y-4 overflow-y-auto custom-scrollbar">
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <div className="text-[11px] font-semibold text-gray-200">{t.posterGenerator.customPromptLabel}</div>
                  <button
                    type="button"
                    onClick={() =>
                ...
                <textarea
                  ...
                  placeholder={t.posterGenerator.videoPromptPlaceholder}
      ...
      {isLightboxOpen && lightboxImage && (
        <div className="fixed inset-0 z-40 bg-black/80 flex items-center justify-center px-4">
          <div className="bg-zinc-900 border border-zinc-700 rounded-2xl w-auto max-w-[90vw] max-h-[90vh] shadow-2xl flex flex-col">
            <div className="flex items-center justify-between px-4 py-3 border-b border-zinc-800">
              <h3 className="text-sm font-semibold text-gray-100">{t.posterGenerator.previewPosterTitle}</h3>
              <button
                type="button"
                onClick={() => {
                  setIsLightboxOpen(false);
            ...
            <div className="flex-1 p-4 flex items-center justify-center overflow-y-auto custom-scrollbar">
              <img
                src={lightboxImage}
                alt={t.posterGenerator.altPreviewFullscreen}
      ...
      {isResetConfirmOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 px-4">
          <div className="bg-zinc-900 border border-zinc-700 rounded-2xl w-full max-w-md shadow-2xl p-5">
            <h2 className="text-base font-semibold text-gray-50 mb-2">{t.posterGenerator.confirmResetTitle}</h2>
            <div className="space-y-2 text-sm text-gray-200 mb-4">
              <p>
                {t.posterGenerator.confirmResetMessage}
              </p>
              <p className="text-gray-400 text-xs">{t.posterGenerator.confirmResetWarning}</p>
            </div>
